...
# GCS에 업로드된 첫 번째 PDF 파일의 URI (실제 파일 경로로 변경)
pdf1_gcs_uri = "gs://shum/problem.pdf" # 예시 파일명 사용
# GCS에 업로드된 두 번째 PDF 파일의 URI (실제 파일 경로로 변경)
pdf2_gcs_uri = "gs://shum/theory.pdf" # 예시 파일명 사용
...
# 첫 번째 PDF 파일 로드 시도
try:
    pdf1_part = Part.from_uri(pdf1_gcs_uri, mime_type="application/pdf")
    contents.append(pdf1_part)
    print(f"첫 번째 PDF 파일 로드 성공: {pdf1_gcs_uri}")
except Exception as e:
    print(f"첫 번째 PDF 파일을 로드하는 데 실패했습니다. URI를 확인하세요: {pdf1_gcs_uri} 오류: {e}")

# 두 번째 PDF 파일 로드 시도
try:
    pdf2_part = Part.from_uri(pdf2_gcs_uri, mime_type="application/pdf")
    contents.append(pdf2_part)
    print(f"두 번째 PDF 파일 로드 성공: {pdf2_gcs_uri}")
except Exception as e:
    print(f"두 번째 PDF 파일을 로드하는 데 실패했습니다. URI를 확인하세요: {pdf2_gcs_uri} 오류: {e}")
